...
apiRouter.post("/create-checkout-session", async (req, res) => {
  try {
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      mode: "subscription", // Changed to subscription for monthly monitoring
...
apiRouter.get("/subscription-status/:email", async (req, res) => {
  try {
    ...
    if (customers.data.length === 0) {
      return res.json({ active: false, message: "Customer not found" });
    }
